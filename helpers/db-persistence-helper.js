/* eslint-disable no-unused-vars, no-param-reassign, no-restricted-syntax */
 ...
 */
const addPathToSettings = (path, settings, baseModel, required = false) => {
  const splitted = path.split(".");
...
exports.addPathToSettings = addPathToSettings;
 ...
 */
const constructGeneralFilter = (settings, baseModel, value, pathsToCompare) => {
  if (!value) {
    return;
  }
  ...
  for (let path of pathsToCompare) {
    ...
    const condition = Sequelize.where(
      ...
      {
        [Sequelize.Op.iLike]: Sequelize.fn("unaccent", `%${value}%`),
      }
    );
    or.push(condition);
  }
  ...
  if (!settings.where[Sequelize.Op.or]) {
    settings.where[Sequelize.Op.or] = [];
  }
  settings.where[Sequelize.Op.or] = [...settings.where[Sequelize.Op.or], ...or];
};

exports.constructGeneralFilter = constructGeneralFilter;
 ...
 * manualmente.
 */
const addFiltersToSettings = (settings, baseModel, filters) => {
  ...
  for (const filter of filters) {
    ...
    for (const path of paths) {
      addPathToSettings(path, settings, baseModel);
      const key = path.split(".").length > 1 ? `$${path}$` : path;
...
exports.addFiltersToSettings = addFiltersToSettings;
 ...
 * ]
 */
const addOrderToSettings = (settings, baseModel, orders) => {
...
exports.addOrderToSettings = addOrderToSettings;
 ...
 * }
 */
const addPagingToSettings = (settings, baseModel, paging) => {
...
exports.addPagingToSettings = addPagingToSettings;
 ...
 * @example
 * <code>
 * const settings = utils.constructSettings({},
 *  db.sequelize.Usuario,
 *  filters,
 *  orders,
 *  paging,
 *  generalFilter);
 ...
 * @param {string} alias
 */
const addIncludeToSetting = (settings, model, alias) => {
  if (!settings.include) {
    settings.include = [];
  }
...
exports.addIncludeToSetting = addIncludeToSetting;
...
exports.constructPathsToCompare = constructPathsToCompare;
 ...
 */
const constructExtendedPathsToCompare = (baseModel, extendedPaths) => [
  ...constructPathsToCompare(baseModel),
  ...extendedPaths,
];

exports.constructExtendedPathsToCompare = constructExtendedPathsToCompare;
